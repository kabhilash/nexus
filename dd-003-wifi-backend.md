...
- **RF-kill.** When rfkill is asserted (hardware switch or `Wifi.Powered = false` via D-Bus), any non-`Gone` state transitions to `Disconnected { RfKilled }`. Because `RfKilled` is not a permanent reason, the state machine would normally cool down to `Idle` — but scans from `Idle` fail while rfkill is asserted, so the interface stays effectively idle until rfkill is released. On release, the next scan tick (or explicit Scan) proceeds normally. See §13.5.
