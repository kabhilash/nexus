...
//! - [`proxy`] — D-Bus proxies + the [`proxy::ManagerOps`] trait
//!   handlers depend on (so tests inject a stub).
//! - [`commands`] — per-subcommand handlers. Each builds a view and
//!   hands off to [`output::render`].
//! - [`output`] — Human / Terse / JSON / Pretty renderers.
//! - [`errors`] + [`errors_map`] — the full DD-008 §9 variant set and
//!   a zbus-error translator.
