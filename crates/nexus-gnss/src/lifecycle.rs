...
/// Emission policy — DD-005 §7.3. Returns `true` when the backend
/// should actually emit `GnssFixChanged` for this `fix`.
pub fn should_emit(entry: &GnssDeviceEntry, fix: &GnssFix, now: Instant) -> Outcome {
...
mod tests {
    ...
    fn emit_suppressed_by_rate_limit() {
        ...
        let got = should_emit(&entry, &good_fix(), Instant::now());
